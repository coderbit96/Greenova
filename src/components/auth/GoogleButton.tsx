"use client";

import { useEffect, useState } from "react";
import { signIn } from "next-auth/react";
import { getProviders } from "next-auth/react";

/**
 * Only renders when Google OAuth is actually configured, so a missing
 * client ID never leaves a dead button on the form.
 */
export default function GoogleButton({ callbackUrl = "/" }: { callbackUrl?: string }) {
  const [enabled, setEnabled] = useState(false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    let active = true;
    getProviders()
      .then((providers) => {
        if (active) setEnabled(Boolean(providers?.google));
      })
      .catch(() => setEnabled(false));
    return () => {
      active = false;
    };
  }, []);

  if (!enabled) return null;

  return (
    <button
      type="button"
      disabled={loading}
      onClick={() => {
        setLoading(true);
        signIn("google", { callbackUrl });
      }}
      className="flex h-12 w-full items-center justify-center gap-3 rounded-full border border-border-base bg-bg-elevated text-sm font-medium text-fg transition-all duration-300 hover:bg-bg-subtle active:scale-[0.98] disabled:opacity-60"
    >
      <svg viewBox="0 0 24 24" className="size-5" aria-hidden>
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1Z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.84Z"
        />
        <path
          fill="#EA4335"
          d="M12 4.75c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 1.46 14.97.5 12 .5a11 11 0 0 0-9.82 6.06l3.66 2.84c.87-2.6 3.3-4.65 6.16-4.65Z"
        />
      </svg>
      Continue with Google
    </button>
  );
}
